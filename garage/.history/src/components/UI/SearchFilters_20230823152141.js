...
const SearchFilters = ({ onSearch }) => {
    const [filtres, setFiltres] = useState({
      ...
      const [marques, setMarques] = useState([]); // État pour stocker les marques

  useEffect(() => {
    fetchMarques(); // Appeler la fonction pour récupérer les marques depuis le backend
  }, []);

  const fetchMarques = async () => {
    try {
      const response = await axios.get("/path-to-your-api/get-marques.php"); // Remplacez par le chemin vers votre script PHP qui récupère les marques
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.post("http://localhost/garageback/front/voiturefiche/all", filtres);
