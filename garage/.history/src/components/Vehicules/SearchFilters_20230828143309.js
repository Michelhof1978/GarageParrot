...
const SearchFilters = ({ onSearch }) => {
  ...
  const handleSearch = async () => {
    try {
      const marque = filtres.marque === "Toutes" ? "" : filtres.marque;
      const response = await axios.get(
        `http://localhost/garageback/front/voiturefiche/all/?marque=${marque}&anneeMin=${filtres.anneeMin}&anneeMax=${filtres.anneeMax}&famille=${filtres.famille.join(",")}&prixMin=${filtres.prixMin}&prixMax=${filtres.prixMax}&kilometrageMin=${filtres.kilometrageMin}&kilometrageMax=${filtres.kilometrageMax}`
      );
