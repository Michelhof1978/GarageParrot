...
const getCurrentYear = () =>  {
  ...
  const handleCheckBoxChange = (e) => {
    ...
    if(checked){
      ...
      setFiltres({ ...filtres, [name]:  filtres.famille.filter((ele) => ele !==  value)})
    }
  
  }
  useEffect(() => {
    fetch("http://localhost/GarageBack/API/vehicule.php?kilometremin=0&kilometremax=200000&anneemin=2000&anneemax=2023&prixmin=0&prixmax=50000").then(res => res.json()).then(data => console.log(data))  }, ["filtres"])
