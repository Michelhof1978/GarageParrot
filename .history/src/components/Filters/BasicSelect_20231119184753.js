...
{/* fonction `map` qui itère sur les options fournies dans le tableau `props.options` */}
      {props.options.map((ele, idx) => { //element , index
        ...
        if(
          Pour chaque option, il vérifie si l'indice actuel (`idx`) est égal à 0. Si c'est le cas, l'option est marquée comme sélectionnée à l'aide de l'attribut `selected`; sinon, c'est une option régulière.
            idx = 0
